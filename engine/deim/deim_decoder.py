...
class MinimalBBoxEnhancer(nn.Module):
    def __init__(self, hidden_dim=256, reg_max=32, alpha=0.1):
        super().__init__()
    ...
    def forward(self, query_features, original_pred):
        ...
        refined_pred = original_pred + self.alpha * bbox_offset
        return refined_pred
...
class LayerQueryMemory(nn.Module):
    ...
    def enhance(self, query_feat: torch.Tensor, memory: torch.Tensor) -> torch.Tensor:
        if memory is None:
            return query_feat
        gate = torch.sigmoid(self.memory_to_gate(memory)).unsqueeze(1)           
    ...
    def update(self, query_feat: torch.Tensor, scores: torch.Tensor = None, prev_memory: torch.Tensor = None) -> torch.Tensor:
        ...
        b, n, c = feat.shape
        if scores is None:
            quality = feat.pow(2).mean(dim=-1)         
        else:
            quality = scores.detach().sigmoid().max(dim=-1).values         

        k = min(max(self.topk, 1), n)
        idx = torch.topk(quality, k=k, dim=1).indices         
        gather_feat = feat.gather(1, idx.unsqueeze(-1).expand(-1, -1, c))           
        pooled = gather_feat.mean(dim=1)         
...
class TransformerDecoder(nn.Module):
    ...
    integrated with Dual Enhancement (Minimal BBox Enhancement & Layer Query Memory).
    """
    def __init__(self, hidden_dim, decoder_layer, decoder_layer_wide, num_layers, num_head, reg_max, reg_scale, up,
        ...
        self.mbe_alpha = float(mbe_alpha)

        print(f"     --- Decoder Configuration ---")
        if self.use_bbox_enhancement:
            self.bbox_enhancer = MinimalBBoxEnhancer(
                hidden_dim=hidden_dim,
                reg_max=reg_max,
                alpha=mbe_alpha
            )
            print(f"     [√] Minimal BBox Enhancement (MBE) Enabled (alpha={mbe_alpha})")
        else:
            print("     [ ] Minimal BBox Enhancement (MBE) Disabled")

        if self.use_lqm:
            self.lqm = LayerQueryMemory(
                hidden_dim=hidden_dim,
                topk=lqm_topk,
                beta=lqm_beta
            )
...
class DEIMTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.lqm_topk = lqm_topk
        self.lqm_beta = lqm_beta
        self.lqm_apply_last_k = lqm_apply_last_k
        self.mbe_alpha = mbe_alpha
